...
int login_c(int fd)
{
	int sockfd = fd;
	char command[40];
//	char copy[]={0xF2,0x06,0x24};
//	bzero(command,40);
//	memcpy(command,copy,3);
//	memcpy(&command[3],g_user,16);
//	memcpy(&command[19],g_passwd,16);
     ...
     while(1)
     {
     	switch(stat)
     	{
     		...
     		//	if(watchdog == 0)
    			...
    			break ;
    		case SENDGPIO:
    			sleep(80); //we couldn't send the signal to the /dev/watchdog,then the soft-watchadog is resert
    			stat = CIRCLE;
    			break;
     	}//switch(stat)
     }//while(1)
