...
printf("1.***$$$$$@@@@####\n");
  ...
  /* always init first */
  	...
  	setsockopt(wd_sock, SOL_SOCKET, SO_REUSEADDR, &SO_REUSEADDR_on, sizeof(SO_REUSEADDR_on)); //Port multiplexing
     	...
     	switch(stat)
     	{
     		...
     		//	if(watchdog == 0)
   			...
   			//		watchdoggpio();		
    			...
    			break ;
    		case SENDGPIO:
    			sleep(80); //we couldn't send the signal to the gpio,then the watchadog is resert
    			stat = CIRCLE;
    			break;
     	}//switch(stat)
     }//while(1)
